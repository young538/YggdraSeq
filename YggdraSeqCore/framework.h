/**
 * @file framework.h
 * @brief MFC/Windows 프레임워크 공통 헤더
 *
 * MFC Extension DLL에서 사용하는 Windows 및 MFC 프레임워크 헤더를 포함합니다.
 * pch.h에서 이 파일을 포함하여 미리 컴파일된 헤더로 활용합니다.
 *
 * @note 이 파일은 프로젝트의 모든 소스 파일에서 간접적으로 포함됩니다.
 * @warning 자주 변경되지 않는 시스템 헤더만 이 파일에 추가하세요.
 */
...
#include <afxcontrolbars.h>     // MFC Feature Pack (CDockablePane 등) 지원

// --- C++ 표준 라이브러리 (자주 사용하는 것만) ---
#include <string>
#include <vector>
#include <map>
#include <memory>
